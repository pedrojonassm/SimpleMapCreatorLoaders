		...
		while (openList.size() > 0) {
			...
			for (int horizontal = -1; horizontal <= 1; horizontal++)
				for (int vertical = -1; vertical <= 1; vertical++) {
					...
					y = current.tile.y;
					z = current.tile.z;
					tile = World.pegar_chao(x + horizontal * SimpleMultiplayerLoader.TileSize,
							y + vertical * SimpleMultiplayerLoader.TileSize, z);
					...
					do {
						...
						gCost = current.gCost - tile.ModificadorVelocidade()
								+ ((vertical != 0 && horizontal != 0) ? 2 : 1) + (tile.isEscada() ? 5 : 0);
