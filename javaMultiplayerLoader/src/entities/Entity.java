...
public class Entity implements tickRender {
	...
	public void tick() {
		if (sqm_alvo != null && sqm_alvo.getZ() == z
			...
			if (this instanceof Player)
				SimpleMultiplayerLoader.player.aPosAtual = SimpleMultiplayerLoader.player.aPosAlvo;
			...
			if (aCaminho.size() > 0) {
				...
				if (SimpleMultiplayerLoader.podeNovaMovimentacao) {
					if (left) {
						if (x - SimpleMultiplayerLoader.TileSize >= 0)
							horizontal = -1;

					} else if (right) {

						if (x + SimpleMultiplayerLoader.TileSize < World.WIDTH * SimpleMultiplayerLoader.TileSize)
							horizontal = 1;
					...
					if (up) {
						...
						if (y - SimpleMultiplayerLoader.TileSize >= 0)
							vertical = -1;

					} else if (down) {

						if (y + SimpleMultiplayerLoader.TileSize < World.HEIGHT * SimpleMultiplayerLoader.TileSize)
							vertical = 1;
			...
			if ((horizontal != 0 || vertical != 0) && !aBloqueadoMovimentacao) {
				...
				sqm_alvo = World.pegar_chao(World.calcular_pos(x + SimpleMultiplayerLoader.TileSize * horizontal,
						y + SimpleMultiplayerLoader.TileSize * vertical, z));
				...
				if (sqm_alvo != null && this instanceof Player)
					SimpleMultiplayerLoader.player.aPosAlvo = sqm_alvo.getaPos();
	...
	public void render(Graphics prGraphics) {
		...
		if (z == SimpleMultiplayerLoader.player.getZ()
				&& (lTileAcima == null || lTileAcima.getZ() >= World.maxRenderingZ || !lTileAcima.tem_sprites())) {
			prGraphics.setColor(Color.WHITE);
			prGraphics.fillRect(x - Camera.x, y - Camera.y, SimpleMultiplayerLoader.TileSize, SimpleMultiplayerLoader.TileSize);
