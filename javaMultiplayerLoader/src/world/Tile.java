...
public class Tile implements tickRender {
	...
	public boolean isTileEmCima(int prX, int prY, int prZ) {
		if (z > prZ) {
			...
			if (maxWidth > SimpleMultiplayerLoader.TileSize || maxHeight > SimpleMultiplayerLoader.TileSize) {
				dx = x - SimpleMultiplayerLoader.TileSize * ((maxWidth / SimpleMultiplayerLoader.TileSize) - 1);
				dy = y - SimpleMultiplayerLoader.TileSize * ((maxHeight / SimpleMultiplayerLoader.TileSize) - 1);
			} else {
				dx = x;
				dy = y;
			}
			dx -= (z - prZ) * SimpleMultiplayerLoader.TileSize;
			dy -= (z - prZ) * SimpleMultiplayerLoader.TileSize;

			if (new Rectangle(dx, dy, maxWidth, maxHeight).intersects(new Rectangle(SimpleMultiplayerLoader.player.getX(),
					SimpleMultiplayerLoader.player.getY(), SimpleMultiplayerLoader.TileSize, SimpleMultiplayerLoader.TileSize)))
				return true;
	...
	private boolean temTileAcima() {
		...
		for (int zz = z + 1; zz < World.maxRenderingZ; zz++) {
			lTile = World.pegar_chao(x + SimpleMultiplayerLoader.TileSize * (zz - z), y + SimpleMultiplayerLoader.TileSize * (zz - z),
					zz);
		...
		if (posicao_Conjunto < CoConjuntoSprites.size() && CoConjuntoSprites.get(posicao_Conjunto) != null)
			for (int iLayer = 0; iLayer < CoConjuntoSprites.get(posicao_Conjunto).getSprites().size(); iLayer++) {
				...
				if (imagens != null && imagens.size() > 0) {
					...
					int dx, dy;
					BufferedImage image = sprite.pegarImagem();
					if (image.getWidth() > SimpleMultiplayerLoader.TileSize || image.getHeight() > SimpleMultiplayerLoader.TileSize) {
						dx = x - Camera.x - SimpleMultiplayerLoader.TileSize;
						dy = y - Camera.y - SimpleMultiplayerLoader.TileSize;
					} else {
						dx = x - Camera.x;
						dy = y - Camera.y;
					}
					dx -= (z - SimpleMultiplayerLoader.player.getZ()) * SimpleMultiplayerLoader.TileSize;
					dy -= (z - SimpleMultiplayerLoader.player.getZ()) * SimpleMultiplayerLoader.TileSize;
					if (z == SimpleMultiplayerLoader.player.getZ() && !temTileAcima()) {
