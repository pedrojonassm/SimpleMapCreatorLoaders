...
public class World {
	...
	public World(File prFile) {
		...
		try {
			...
			if (prFile != null && prFile.exists()) {
				...
				WIDTH = SimpleMultiplayerLoader.aConfig.getWorldWidth();
				HEIGHT = SimpleMultiplayerLoader.aConfig.getWorldHeight();
				HIGH = SimpleMultiplayerLoader.aConfig.getWorldHigh();
				aArquivo = prFile;
				ok = true;
			}
	...
	public void tick() {
		...
		int xfinal = xstart + (SimpleMultiplayerLoader.windowWidth >> log_ts) + 2;
		int yfinal = ystart + (SimpleMultiplayerLoader.windowHEIGHT >> log_ts) + 2;

		if ((xstart -= (SimpleMultiplayerLoader.player.getZ() + 1)) < 0)
			xstart = 0;
		if ((ystart -= (SimpleMultiplayerLoader.player.getZ() + 1)) < 0)
			ystart = 0;
		...
		for (int xx = xstart; xx <= xfinal; xx++)
			for (int yy = ystart; yy <= yfinal; yy++) {
				...
				Tile lTile = tiles[(xx + (yy * WIDTH)) * HIGH + SimpleMultiplayerLoader.player.getZ()];
				if (lTile != null)
					lTile.tick();
			}
	...
	public void render(Graphics g) {
		...
		for (int xx = SimpleMultiplayerLoader.player.getX() >> log_ts + 1; xx <= xfinal && !lBreak; xx++)
			for (int yy = SimpleMultiplayerLoader.player.getY() >> log_ts + 1; yy <= yfinal && !lBreak; yy++)
				for (int zz = 1; zz < HIGH - SimpleMultiplayerLoader.player.getZ() - 1 && !lBreak; zz++) {
					if (xx < 0 || yy < 0 || xx >= WIDTH || yy >= HEIGHT) {
						continue;
					}
					lTile = tiles[(xx + (yy * WIDTH)) * HIGH + zz];

					if (lTile != null && lTile.isTileEmCima(SimpleMultiplayerLoader.player.getX(),
							SimpleMultiplayerLoader.player.getY(), SimpleMultiplayerLoader.player.getZ())) {
		...
		for (int xx = xstart; xx <= xfinal; xx++) {
			for (int yy = ystart; yy <= yfinal; yy++) {
				for (int zz = 0; zz < maxRenderingZ; zz++) {
					...
					if (lTile != null) {
						lTile.render(g);
						if (lTile.getaPos() == SimpleMultiplayerLoader.player.aPosAtual
								|| lTile.getaPos() == SimpleMultiplayerLoader.player.aPosAlvo)
							SimpleMultiplayerLoader.player.render(g);
					}
				}
	...
	public static void novo_mundo(File file) {
		ready = false;
		SimpleMultiplayerLoader.aConfig = new ExConfig();
		SimpleMultiplayerLoader.world = new World(file);
		SimpleMultiplayerLoader.instance.startGerador();
	}
