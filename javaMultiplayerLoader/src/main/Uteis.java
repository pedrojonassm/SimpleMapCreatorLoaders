...
public class Uteis {
	...
	public static int[] calcularPosicaoSemAltura(int prPos) {
		int[] retorno = { 0, 0, 0 };
		retorno[0] = (int) ((prPos % (World.WIDTH * World.HIGH)) / World.HIGH) * SimpleMultiplayerLoader.TileSize - Camera.x;
		retorno[1] = (int) (prPos / World.HEIGHT / World.HIGH) * SimpleMultiplayerLoader.TileSize - Camera.y;
	...
	public static int[] calcularPosicaoComAltura(int prPos) {
		...
		int lSubtract = (prPos % World.HIGH) * SimpleMultiplayerLoader.TileSize;
		retorno[0] -= lSubtract;
		retorno[1] -= lSubtract;
		return retorno;
	}
