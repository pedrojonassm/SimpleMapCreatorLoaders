...
public class Astar {
        ...
        while (openList.size() > 0) {
            ...
            for (int horizontal = -1; horizontal <= 1; horizontal++)
                for (int vertical = -1; vertical <= 1; vertical++) {
                    ...
                    y = current.tile.y;
                    z = current.tile.z;
                    tile = World.pegar_chao(x + horizontal * OnlineMapLoader.TileSize, y + vertical * OnlineMapLoader.TileSize, z);
                    ...
                    do {
                        ...
                        gCost = current.gCost - tile.ModificadorVelocidade() + ((vertical != 0 && horizontal != 0) ? 2 : 1)
                                + (tile.isEscada() ? 5 : 0);

                        if (gCost < 0)
                            gCost = ((vertical != 0 && horizontal != 0) ? 2 : 1) + (tile.isEscada() ? 5 : 0);
    ...
    public static boolean tileEqualsVector(Vector2i prVector2i, Tile prTile) {
        return (prVector2i.x == prTile.getX() && prVector2i.y == prTile.getY() && prVector2i.z == prTile.getZ());
    }
